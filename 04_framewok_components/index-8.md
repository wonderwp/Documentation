...
All the public methods defined in this service can be used as an ajax endpoint via the Basic ajax calls via the old admin-ajax endpoint (wp-admin/admin-ajax.php). No more hook definition, you just create the API service, and then declare public methods in it. Those methods can now be accessed via JavaScript calls. You can also annotate your methods to make them available in the new REST API.

Most of the concepts underlying the API component are described on the [API service page](../../02_Plugin_development/04_Services/04_Api_service.md). You'll get plenty of details there.

...
The framework provides a default implementation of the requester that you can use. It is available as a service in the container under the `wwp.http.requester` key. It is based on WordPress http methods and uses the `wp_remote_*` functions to make the requests. It returns a `WonderWp\Component\Http\Response` object that you can use to get the response body, headers, status code, etc.
